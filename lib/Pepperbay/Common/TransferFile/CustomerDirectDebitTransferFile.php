...
class CustomerDirectDebitTransferFile extends BaseTransferFile
{
     ...
     *
     * @throws \Pepperbay\Sepa\Exception\InvalidTransferTypeException
     */
        ...
        foreach ($this->paymentInformations as $payment) {
            ...
            foreach ($payment->getTransfers() as $transfer) {
                if (!$transfer instanceof CustomerDirectDebitTransferInformation) {
                    throw new InvalidTransferTypeException('Transfers must be of type \Pepperbay\Sepa\TransferInformation\CustomerDirectDebitTransferInformation instead of: ' . get_class(
                        $transfer
                    ));
                }
            }
