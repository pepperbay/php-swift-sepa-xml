...
namespace Pepperbay\Common\TransferFile;

use Pepperbay\Common\Exception\InvalidTransferFileConfiguration;
use Pepperbay\Common\Exception\InvalidTransferTypeException;
...
class CustomerDirectDebitTransferFile extends BaseTransferFile
{
     ...
     *
     * @throws \Pepperbay\Common\Exception\InvalidTransferTypeException
     */
        ...
        foreach ($this->paymentInformations as $payment) {
            ...
            foreach ($payment->getTransfers() as $transfer) {
                if (!$transfer instanceof CustomerDirectDebitTransferInformation) {
                    throw new InvalidTransferTypeException('Transfers must be of type \Pepperbay\Common\TransferInformation\CustomerDirectDebitTransferInformation instead of: ' . get_class(
                        $transfer
                    ));
                }
            }
